...
export const StatisticTemplate:FC<Props> = () => {
  ...
  const selectedDepartmentObject = selectedDepartment !== null
    ? departmentList.find(department => department.id === selectedDepartment)
    : null;

  // Retrieve selected group object
  const selectedGroupObject = selectedDepartmentObject && selectedGroup !== null
    ? selectedDepartmentObject.groups.find(group => group.id === selectedGroup)
    : null;
  ...
  return (
    <div>
      <div>
        ...
        <select style={{ margin: "5px" }} id="department" name="department"
        ...
        <label style={{ margin: "5px" }} htmlFor="group">Bộ môn: </label>
        <select style={{ margin: "5px" }} id="group" name="group"
          value={selectedGroup || ""}
          onChange={handleGroupChange}
        >
          <option value="">Chọn bộ môn</option>
          {selectedDepartmentObject && selectedDepartmentObject.groups.map(group => (
